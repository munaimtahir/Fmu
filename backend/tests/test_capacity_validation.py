...
class TestEnrollmentCapacity:
    def test_enroll_within_capacity(self, api_client, sample_section_with_capacity, sample_students):
    ...
    def test_enroll_at_full_capacity(self, api_client, sample_section_with_capacity, sample_students):
        """Test enrolling when section is at capacity"""
        section = sample_section_with_capacity

        # Enroll two students (capacity = 2)
        Enrollment.objects.create(student=sample_students[0], section=section, status="enrolled")
        Enrollment.objects.create(student=sample_students[1], section=section, status="enrolled")
    ...
    def test_dropped_enrollment_frees_slot(self, api_client, sample_section_with_capacity, sample_students):
        """Test that dropped enrollment doesn't count toward capacity"""
        section = sample_section_with_capacity

        # Enroll two students
        enrollment1 = Enrollment.objects.create(student=sample_students[0], section=section, status="enrolled")
        Enrollment.objects.create(student=sample_students[1], section=section, status="enrolled")
