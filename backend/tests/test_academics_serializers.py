...
def test_section_serializer_accepts_teacher_name_string():
    ...
    course = Course.objects.create(code="ENG100", title="Intro", credits=3, program=program)
...
def test_section_serializer_handles_teacher_relationship():
    ...
    course = Course.objects.create(code="MATH101", title="Calculus", credits=4, program=program)
...
def test_section_serializer_create_handles_string_validated_data():
    ...
    course = Course.objects.create(code="PHY100", title="Mechanics", credits=3, program=program)

    serializer = SectionSerializer()
    section = serializer.create(
        {"course": course, "term": "Winter-26", "teacher": "Dr. Who", "teacher_name": ""}
    )
