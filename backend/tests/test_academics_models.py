...
def test_academics_models_str_and_save_behaviour():
    term = Term.objects.create(
        name="Fall 2025", status="open", start_date=date(2025, 1, 1), end_date=date(2025, 6, 1)
    )
    ...
    section = Section.objects.create(course=course, term="Fall-25", teacher=teacher, capacity=40)
...
def test_section_duplicate_teacher_name_update_raises():
    ...
    course = Course.objects.create(code="BA101", title="Intro", credits=3, program=program)
    original = Section.objects.create(course=course, term="Fall-25", teacher="Dr. Same")
