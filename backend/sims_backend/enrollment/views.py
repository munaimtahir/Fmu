...
class EnrollmentViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=["post"], url_path="sections/(?P<section_id>[^/.]+)/enroll")
    def enroll_in_section(self, request, section_id=None):
        ...
        if Enrollment.objects.filter(student=student, section=section).exists():
            return Response(
                {"error": {"code": 409, "message": "Student is already enrolled in this section"}},
